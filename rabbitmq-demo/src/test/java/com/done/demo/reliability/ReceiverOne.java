...
public class ReceiverOne {
    ...
    public void receive() throws Exception {
        //区分不同工作进程的输出
        int hashCode = ReceiverOne.class.hashCode();
        //创建连接和频道
        ...
        while (true) {
            ...
            try{
                log.info(hashCode + " Received Message：'" + message + "'" +" applyId="+delivery.getProperties().getAppId());//处理业务逻辑
                channel.basicAck(delivery.getEnvelope().getDeliveryTag(),false);//消费成功
            }catch(Exception e){
                /**
                 * 第三个参数(requeue)：表示是否放回queue中。
                 */
                channel.basicNack(delivery.getEnvelope().getDeliveryTag(),false,true);//消费失败,rabbitmq重新投递
            }
        }
    }
}
